...
`selection_table()` ranks stable terms for each parameter; `plot_sb_gamlss()` overlays selection frequency vs. coefficient magnitude, and `effect_plot()` visualizes partial effects (using **ggplot2** if available, otherwise base R).
...
head(selection_table(fit_grouped))
```
